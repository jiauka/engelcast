/*
 * roomy-repeater.c - mDNS repeater daemon
...
roomy-repeater -f eth1 wlan0:192.168.67.10 wlan0-1:192.168.67.147 wlan0-2:192.168.67.12 wlan0-3:192.168.67.13
roomy-repeater -s -f eth1 wlan0:192.168.67.129 wlan0-1:192.168.67.205 wlan0-2:192.168.67.12 wlan0-3:192.168.67.13
...
struct if_sock {
    ...
    struct sockaddr_in fixaddr;
};

int server_sockfd = -1;
static 	int is_suiteroom = 0;
...
static void roomy_repeater_shutdown(int sig) {
	shutdown_flag = 1;
}
...
static void daemonize() {
	...
	signal(SIGTERM, roomy_repeater_shutdown);
...
static void show_help(const char *progname) {
	fprintf(stderr, "RoomyCC repeater\n");
	fprintf(stderr, "Copyright (C) 2017 Jaume Clarens\n");

	fprintf(stderr, "usage: %s [ -f ] <eth0>  <wifi1>:IP <wifi2>:IP ...\n", progname);
	fprintf(stderr, "\n"
					"<eth0> specifies an interface like \"eth0\"\n"
					"packets received on an interface is repeated across all other specified interfaces \n"
					"returning packets from wifi interface only go back for especified IP\n"
					"maximum number of wifi interfaces is 4\n"
					"\n"
					" flags:\n"
					"	-f	runs in foreground for debugging\n"
					"	-p	specifies the pid file path (default: " PIDFILE ")\n"
					"	-s	suite room, all Chromecast are viewed from all wifis\n"
					"	-h	shows this help\n"
					"\n"
		);
}
...
static int parse_opts(int argc, char *argv[]) {
	int c;
	int help = 0;
	while ((c = getopt(argc, argv, "shfp:")) != -1) {
		switch (c) {
			case 'h': help = 1; break;
			case 's': is_suiteroom = 1; break;
...
int main(int argc, char *argv[]) {
	...
	for (i = optind; i < argc; i++) {
		char *iface = strtok( argv[i], ":");
		char *ip = strtok(NULL, ":");
		if(ip != NULL) {
			socks[num_socks].fixaddr.sin_addr.s_addr=inet_addr(ip);
		}
		printf("iface = %s, ip = %s\n",iface,ip);
	...
	while (! shutdown_flag) {
		...
		if (FD_ISSET(server_sockfd, &sockfd_set)) {
			...
			for (j = 0; j < num_socks; j++) {
				...
				if ((socks[j].fixaddr.sin_addr.s_addr == fromaddr.sin_addr.s_addr) || (j==0) || (is_suiteroom == 1)) {
					if (foreground)
						printf("repeating data to %s from %s\n", socks[j].ifname, inet_ntoa(fromaddr.sin_addr));
					...
					if (foreground)
						printf("NOT repeating data to %s from %s\n", socks[j].ifname, inet_ntoa(fromaddr.sin_addr));
				}
