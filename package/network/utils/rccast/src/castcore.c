...
bool SendCastMessage(struct sCastCtx *Ctx, char *ns, char *dest, char *payload, ...)
{
	...
	if (!stristr(message.payload_utf8, "PING")) {
		LOG_DEBUG("Cast sending: %s", message.payload_utf8);
	}
	...
	switch (Ctx->Status) {
		...
		case CAST_CONNECTED:
			if (!Ctx->waitId) {
				...
				LOG_INFO(" Launching receiver %d", Ctx->waitId);
			} else {
			...
			break;
		default:
			LOG_INFO(" unhandled state %d",  Ctx->Status);
			break;
	}
...
bool CastConnect(struct sCastCtx *Ctx)
{
	...
	if (err) {
		...
		LOG_ERROR(" Cannot open socket connection to %s:%d(%d)",  inet_ntoa(Ctx->ip),Ctx->port,err);
		pthread_mutex_unlock(&Ctx->Mutex);
		return false;
	}
	...
	if (SSL_connect(Ctx->ssl)) {
		LOG_INFO(" SSL connection opened [%p]",  Ctx->ssl);
	}
	else {
		err = SSL_get_error(Ctx->ssl,err);
		LOG_ERROR(" Cannot open SSL connection (%d)",  err);
		closesocket(Ctx->sock);
		pthread_mutex_unlock(&Ctx->Mutex);
		return false;
	}
...
bool UpdateCastDevice(struct sCastCtx *Ctx, struct in_addr ip, u16_t port)
{
	if (Ctx->port != port || Ctx->ip.s_addr != ip.s_addr) {
		LOG_INFO(" changed ip:port %s:%d", Ctx, inet_ntoa(ip), port);
		pthread_mutex_lock(&Ctx->Mutex);
		Ctx->ip	= ip;
		Ctx->port = port;
		pthread_mutex_unlock(&Ctx->Mutex);
...
void ProcessQueue(tCastCtx *Ctx) {
	...
	if (!strcasecmp(item->Type, "GET_MEDIA_STATUS") && Ctx->mediaSessionId) {
		...
		LOG_INFO(" Processing GET_MEDIA_STATUS (id:%u)",  Ctx->waitId);
	...
	if (!strcasecmp(item->Type, "GET_STATUS")) {
		...
		LOG_INFO(" Processing GET_STATUS (id:%u)",  Ctx->waitId);
	...
	if (!strcasecmp(item->Type, "SET_VOLUME")) {
		...
		LOG_INFO(" Processing VOLUME (id:%u)",  Ctx->waitId);
	}
	...
	if (!strcasecmp(item->Type, "PLAY") || !strcasecmp(item->Type, "PAUSE")) {
		if (Ctx->mediaSessionId) {
			...
			LOG_INFO(" Processing %s (id:%u)",  item->Type, Ctx->waitId);
			...
			LOG_WARN(" PLAY un-queued but no media session");
		}
	}
	...
	if (!strcasecmp(item->Type, "LOAD")) {
		...
		LOG_INFO(" Processing LOAD (id:%u)",  Ctx->waitId);

		msg = json_pack("{ss,si,ss,sf,sb,ss,so}", "type", "LOAD",
						"requestId", Ctx->waitId, "sessionId", Ctx->sessionId,
						"currentTime", 0.0, "autoplay", 1,
		...
		if (now - last > 3000 && Ctx->Status != CAST_DISCONNECTED) {
			...
			if (Ctx->ssl) {
				...
				if (now - Ctx->lastPong > 15000) {
					LOG_INFO(" No response to ping", Ctx);
					CastDisconnect(Ctx);
				}
			}
...
static void *CastSocketThread(void *args)
{
	...
	while (Ctx->running) {
		...
		if (!GetNextMessage(&Ctx->sslMutex, Ctx->ssl, &Message)) {
			LOG_WARN(" SSL connection closed", Ctx);
			CastDisconnect(Ctx);
			continue;
		}

		root = json_loads(Message.payload_utf8, 0, &error);
		LOG_SDEBUG(" %s",  json_dumps(root, JSON_ENCODE_ANY | JSON_INDENT(1)));
		...
		if (json_is_string(val)) {
			...
			if (!strcasecmp(str, "MEDIA_STATUS")) {
				LOG_DEBUG(" type:%s (id:%d) %s",  str, requestId, GetMediaItem_S(root, 0, "playerState"));
			}
			else if (strcasecmp(str, "PONG") || *loglevel == lSDEBUG) {
				LOG_DEBUG(" type:%s (id:%d)",  str, requestId);
			}
			...
			LOG_SDEBUG(" recvID %u (waitID %u)", Ctx, requestId, Ctx->waitId);
			...
			if (Ctx->waitId && Ctx->waitId == requestId) {
				...
				if (!strcasecmp(str,"MEDIA_STATUS") && Ctx->waitMedia == requestId) {
					...
					if (id) {
						...
						LOG_INFO(" Media session id %d",  Ctx->mediaSessionId);
						// set media volume when session is re-connected
						SetMediaVolume(Ctx, Ctx->MediaVolume);
					}
					else {
						LOG_ERROR(" waitMedia match but no session %u",
							 Ctx->waitMedia);
					}
